...
export default function CheckStatsTab({ checksData, flota, isMobile }) {
  ...
  // ── Mapa nombre completo del equipo → id_corto ───────────────
  // Ej: "AE N°19 TOYOTA" → "AE-19"
  const equipoIdMap = useMemo(() => {
    const m = {};
    flota.forEach((eq) => {
      if (!eq.name || !eq.id) return;
      const norm = eq.name.trim().toUpperCase().replace(/N[°º]/g, "N°").replace(/\s+/g, " ");
      m[norm] = eq.id;
      m[eq.name.trim().toUpperCase()] = eq.id;
    });
    return m;
  }, [flota]);

  // Resuelve el id_corto de un check usando el mapa
  const resolveEquipoId = (equipo) => {
    if (!equipo) return "Sin equipo";
    const raw  = equipo.trim().toUpperCase();
    const norm = raw.replace(/N[°º]/g, "N°").replace(/\s+/g, " ");
    return equipoIdMap[norm] || equipoIdMap[raw] || equipo;
  };

  // ── Abreviatura de operario: "BELMONTE ALLENDE JUAN E." ──────
  // Formato en DB: "APELLIDO NOMBRE1 NOMBRE2" (o variantes)
  // Salida: "BELMONTE A.J.E." → primer palabra completa + iniciales del resto
  const fmtOperario = (nombre) => {
    if (!nombre || nombre.trim() === "" || nombre === "Sin operario" || nombre === "OTRO") return nombre;
    const parts = nombre.trim().split(/\s+/);
    if (parts.length <= 2) return nombre; // "GARCIA JUAN" → sin cambios
  ...
  const byEquipo = useMemo(() => {
    const m = {};
    filtered.forEach((c) => {
      const key = resolveEquipoId(c.equipo);
      m[key] = (m[key] || 0) + 1;
    });
    return Object.entries(m).sort((a, b) => b[1] - a[1]);
  }, [filtered, equipoIdMap]);
  ...
  const bySectorEquipo = useMemo(() => {
    const m = {};
    filtered.forEach((c) => {
      const id = resolveEquipoId(c.equipo);
      const eq = flota.find(e => e.id === id);
      const sector = eq?.sector || "Sin asignar";
      m[sector] = (m[sector] || 0) + 1;
    });
    return Object.entries(m).sort((a, b) => b[1] - a[1]);
  }, [filtered, equipoIdMap, flota]);

  // ── KPIs ─────────────────────────────────────────────────────
  const totalChecks    = filtered.length;
  const equiposUnicos  = new Set(filtered.map(c => resolveEquipoId(c.equipo))).size;
  ...
  const activeData = { equipo:byEquipo, operario:byOperario, sector_operario:bySectorOperario, sector_equipo:bySectorEquipo }[view];
  ...
  // ── Label a mostrar en la barra (abreviado) y en la tabla (completo) ──
  const displayLabel = (label) => {
    if (view === "operario") return fmtOperario(label);
    return label; // equipo ya es ID, sector es corto
  };
            ...
            {activeData.map(([label, count]) => {
              ...
              return (
                <div key={label} style={s.barRow}>
                  {/* Label — ancho fijo, alineado a la derecha para quedar pegado a la barra */}
                  <div style={s.barLabel} title={label}>{shown}</div>
                  <div style={s.barTrack}>
                    <div style={{ ...s.barFill, width: `${ratio * 100}%`, background: barColor(ratio) }}/>
                  </div>
              ...
              <tbody>
                {activeData.map(([label, count], i) => (
                  <tr key={label} style={s.tr}>
                    ...
                    {/* En la tabla mostramos el label completo */}
...
const s = {
  ...
  kpiCard: { flex:"1 1 130px", minWidth:110, padding:"14px 16px", background:"var(--bg-surface)", border:"1px solid", borderRadius:"var(--radius-md)", display:"flex", flexDirection:"column", gap:5 },
  ...
  chartTitle: { fontSize:13, fontWeight:600, color:"var(--text-primary)", marginBottom:14, display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:6 },
  ...
  barRow: { display:"flex", alignItems:"center", gap:8, minHeight:30 },
  // Label alineado a la DERECHA → el texto queda pegado al inicio de la barra
  // sin espacio vacío entre el fin del texto y el comienzo de la barra
  barLabel: {
    fontSize:11, fontWeight:600, fontFamily:"var(--font-mono)",
    color:"var(--text-secondary)",
    width:100,          // ancho fijo suficiente para IDs y abreviaturas
    flexShrink:0,
    textAlign:"right",  // ← clave: texto pegado a la barra
    overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap",
  },
  barTrack: { flex:1, height:22, background:"var(--bg-surface-2)", borderRadius:"var(--radius-xs)", overflow:"hidden" },
  barFill:  { height:"100%", borderRadius:"var(--radius-xs)", transition:"width 400ms ease" },
  barCount: { fontSize:12, fontWeight:700, fontFamily:"var(--font-mono)", width:30, textAlign:"right", flexShrink:0 },
