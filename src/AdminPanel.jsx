...
export default function AdminPanel({ onBack }) {
  ...
  const TABS = [
    { key: "flota",    label: "Flota",      icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg> },
    { key: "personal", label: "Personal",   icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg> },
    { key: "calendar", label: "Calendario", icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg> },
  ];
    ...
    <div>
      ...
      <div style={s.tableWrap}>
        <table style={s.table}>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id} style={{ ...s.tr, opacity: row.activo ? 1 : 0.4 }}>
                <td style={{ ...s.td, fontFamily: "var(--font-mono)", fontWeight: 600, color: "var(--accent)", fontSize: 13 }}>{row.id_corto}</td>
          ...
          <tbody>
            {filtered.map((row) => (
              <tr key={row.id} style={{ ...s.tr, opacity: row.activo ? 1 : 0.4 }}>
                ...
                <td style={s.td}>
                  <select style={{ ...s.select, padding: "4px 8px", fontSize: 12 }} value={row.sector || ""}
            ...
            <tbody>
              {filteredTemplate.map((row) => (
                <tr key={row.id} style={s.tr}>
                  <td style={{ ...s.td, color: "var(--accent)", fontWeight: 600 }}>{row.dia}</td>
                  <td style={{ ...s.td, fontFamily: "var(--font-mono)", fontSize: 12 }}>{row.equipo}</td>
                  ...
                  <tr key={row.id} style={s.tr}>
                    ...
                    <td style={s.td}>
                      {row.inicio && row.fin
                        ...
                        : <span style={{ color: "var(--danger)", fontWeight: 600 }}>Cancelado</span>}
                    </td>
  ...
  if (!editing) {
    return (
      <span onClick={() => setEditing(true)}
        style={{ cursor: "pointer", borderBottom: "1px dashed var(--border-hover)", padding: "2px 4px", color: "var(--text-primary)", fontSize: 12, fontFamily: "var(--font-mono)", display: "inline-flex", alignItems: "center", gap: 6, transition: "all 150ms" }}>
        {val || <span style={{ color: "var(--text-muted)" }}>—</span>}
        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="var(--text-muted)" strokeWidth="2" style={{ opacity: 0.5 }}><path d="M17 3a2.83 2.83 0 114 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>
  ...
  return (
    <button onClick={() => onChange(!value)}
      style={{
        ...
        background: value ? "var(--ok)" : "var(--bg-surface-3)", position: "relative",
...
function Loader() { return <div style={{ color: "var(--text-secondary)", padding: 32, textAlign: "center", fontSize: 13, fontFamily: "var(--font-ui)" }}>Cargando...</div>; }
function Toast({ msg }) {
  return <div style={{ padding: "8px 16px", background: "var(--ok-bg)", border: "1px solid var(--ok-border)", borderRadius: "var(--radius-sm)", color: "var(--ok)", fontSize: 12, marginBottom: 16, display: "inline-block", fontFamily: "var(--font-ui)" }}>{msg}</div>;
}
...
const s = {
  ...
  logo: { width: 40, height: 40, display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg-surface-2)", borderRadius: "var(--radius-sm)", border: "1px solid var(--accent-dim)" },
  headerTitle: { fontSize: 15, fontWeight: 700, color: "var(--text-primary)" },
  headerSub: { fontSize: 11, color: "var(--text-secondary)", marginTop: 2 },
  backBtn: { background: "transparent", border: "1px solid var(--border)", color: "var(--text-secondary)", padding: "6px 14px", borderRadius: "var(--radius-sm)", cursor: "pointer", fontFamily: "var(--font-ui)", fontSize: 12, fontWeight: 500 },
  tabs: { display: "flex", gap: 2, padding: "0 24px", background: "var(--bg-surface)", borderBottom: "1px solid var(--border)" },
  tab: { padding: "12px 20px", fontSize: 13, fontWeight: 500, background: "transparent", color: "var(--text-secondary)", border: "none", borderBottom: "2px solid transparent", cursor: "pointer", fontFamily: "var(--font-ui)", display: "flex", alignItems: "center", transition: "all 200ms" },
  tabActive: { color: "var(--accent)", borderBottomColor: "var(--accent)", background: "var(--accent-dim)" },
  content: { padding: "24px" },
  sectionTitle: { margin: 0, fontSize: 18, fontWeight: 700, color: "var(--text-primary)" },
  sectionCount: { fontSize: 12, color: "var(--text-secondary)", marginTop: 2 },
  subSection: { marginBottom: 32 },
  subTitle: { margin: 0, fontSize: 14, fontWeight: 600, color: "var(--text-primary)" },
  subCount: { fontSize: 11, color: "var(--text-secondary)", background: "var(--bg-surface-2)", padding: "2px 8px", borderRadius: "var(--radius-pill)", marginLeft: 8, fontWeight: 400 },
  ...
  badge: { padding: "3px 8px", borderRadius: "var(--radius-sm)", fontSize: 11, background: "var(--bg-surface-2)", color: "var(--text-secondary)", fontWeight: 500 },
  label: { fontSize: 11, color: "var(--text-tertiary)", textTransform: "uppercase", letterSpacing: 0.5, fontWeight: 600 },
  input: { padding: "8px 12px", fontSize: 12, background: "var(--bg-surface-2)", color: "var(--text-primary)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", fontFamily: "var(--font-ui)", width: "100%", transition: "border-color 150ms" },
  ...
  addBtn: { padding: "8px 16px", fontSize: 12, fontWeight: 600, background: "var(--info-bg)", color: "var(--info)", border: "1px solid var(--info-border)", borderRadius: "var(--radius-sm)", cursor: "pointer", fontFamily: "var(--font-ui)", transition: "all 150ms" },
  addForm: { background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-md)", padding: "18px 20px", marginBottom: 20 },
  ...
  saveBtn: { padding: "8px 20px", fontSize: 12, fontWeight: 600, background: "var(--ok-bg)", color: "var(--ok)", border: "1px solid var(--ok-border)", borderRadius: "var(--radius-sm)", cursor: "pointer", fontFamily: "var(--font-ui)" },
  cancelBtn: { padding: "8px 16px", fontSize: 12, background: "transparent", color: "var(--text-secondary)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", cursor: "pointer", fontFamily: "var(--font-ui)" },
  deleteBtn: { padding: "4px 10px", fontSize: 11, background: "var(--danger-bg)", color: "var(--danger)", border: "1px solid var(--danger-border)", borderRadius: "var(--radius-sm)", cursor: "pointer", fontFamily: "var(--font-ui)" },
  miniSaveBtn: { padding: "3px 8px", fontSize: 11, background: "var(--ok-bg)", color: "var(--ok)", border: "none", borderRadius: "var(--radius-sm)", cursor: "pointer", fontFamily: "var(--font-ui)" },
  miniCancelBtn: { padding: "3px 8px", fontSize: 11, background: "var(--bg-surface-3)", color: "var(--text-secondary)", border: "none", borderRadius: "var(--radius-sm)", cursor: "pointer", fontFamily: "var(--font-ui)" },
  empty: { padding: "24px", textAlign: "center", color: "var(--text-muted)", background: "var(--bg-surface)", borderRadius: "var(--radius-md)", border: "1px solid var(--border)", fontSize: 13 },
  loginWrap: { fontFamily: "var(--font-ui)", background: "var(--bg-base)", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" },
  loginBox: { background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", padding: "40px 36px", width: "100%", maxWidth: 380, textAlign: "center" },
  loginIcon: { width: 56, height: 56, display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg-surface-2)", borderRadius: "var(--radius-md)", border: "1px solid var(--accent-dim)", margin: "0 auto 20px" },
  loginTitle: { margin: "0 0 8px", fontSize: 20, fontWeight: 700, color: "var(--text-primary)" },
  loginSub: { margin: "0 0 24px", fontSize: 13, color: "var(--text-secondary)" },
  loginInput: { padding: "10px 14px", fontSize: 14, background: "var(--bg-surface-2)", color: "var(--text-primary)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", fontFamily: "var(--font-ui)", width: "100%", marginBottom: 8, textAlign: "center", letterSpacing: 4, transition: "border-color 150ms" },
  loginError: { fontSize: 12, color: "var(--danger)", marginBottom: 12 },
  loginBtn: { width: "100%", padding: "11px", fontSize: 13, fontWeight: 700, background: "var(--accent)", color: "var(--bg-base)", border: "none", borderRadius: "var(--radius-sm)", cursor: "pointer", fontFamily: "var(--font-ui)", transition: "background 150ms" },
};
